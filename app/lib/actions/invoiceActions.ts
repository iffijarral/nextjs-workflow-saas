...
export async function createInvoice(
  ...
  for (let i = 0; ; i++) {
    ...
    services.push({
      ...
      serviceName:
        formData.get(`services[${i}][serviceName]`)?.toString() ?? "",
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      for (const s of validServices) {
        ...
        if (!finalServiceId) {
          throw new Error(
            "Missing serviceId for invoice item. Service name was also not provided.",
          );
        }

        await tx.invoiceService.create({
          // Use tx for transaction
          data: {
    ...
    return {
      // This return will likely not be reached due to redirect
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      for (const s of validServices) {
        ...
        if (!finalServiceId) {
          throw new Error(
            "Missing serviceId for updated invoice item. Service name was also not provided.",
          );
        }
