...
export async function deleteCustomer(id: string) {
  try {
    ...
    if (!validatedFields.success) {
      console.error(
        "Validation failed for deleteCustomer ID:",
    ...
    await prisma.$transaction(async (tx) => {
      ...
      console.log(
        `Customer ${customerId} and its projects soft-deleted and marked as cancelled successfully.`,
      );
    });
    ...
    return {
      ...
      success: true,
      message:
        "Customer and associated projects successfully marked as deleted.",
    };
  } catch (error) {
