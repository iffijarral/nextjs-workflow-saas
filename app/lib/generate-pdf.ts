...
export default async function Faktura(invoice: InvoiceTableRow) {
  ...
  page.drawText(`${invoice.project.customer.name}`, {
    x: margin,
    y: yPosition,
    size: headFontSize,
    font,
  });
  page.drawText(`${invoice.project.customer.email}`, {
    x: margin,
    y: yPosition - 20,
    size: headFontSize,
    font,
  });
  page.drawText(`${invoice.project.customer.address?.street}`, {
    x: margin,
    y: yPosition - 40,
    size: headFontSize,
    font,
  ...
  page.drawText(
    `${invoice.project.customer.address?.postalCode}, ${invoice.project.customer.address?.city}`,
    { x: margin, y: yPosition - 60, size: headFontSize, font },
  );
  ...
  page.drawText("Tingbjerg ås 9", {
    x: rightX,
    y: yPosition - 20,
    size: headFontSize,
    font,
  });
  page.drawText("2700 Brønshøj", {
    x: rightX,
    y: yPosition - 40,
    size: headFontSize,
    font,
  });
  page.drawText("CVR. 12345678", {
    x: rightX,
    y: yPosition - 60,
    size: headFontSize,
    font,
  });
  page.drawText("rahmaler.dk", {
    x: rightX,
    y: yPosition - 80,
    size: headFontSize,
    font,
  });
  page.drawText("info@rahmaler.dk", {
    x: rightX,
    y: yPosition - 100,
    size: headFontSize,
    font,
  });
  page.drawText("Telefon 70 10 20 31", {
    x: rightX,
    y: yPosition - 120,
    size: headFontSize,
    font,
  });
  page.drawText(`Dato: ${fakturaDate}`, {
    x: rightX,
    y: yPosition - 140,
    size: headFontSize,
    font,
  ...
  invoice.services.forEach((s: InvoiceService, index: number) => {
    ...
    page.drawText(`${s.quantity}`, {
      x: margin + 200,
      y: yPosition,
      size: 12,
      font,
    });
    page.drawText(`${(s.amount / 100).toFixed(2)}`, {
      x: margin + 300,
      y: yPosition,
      size: 12,
      font,
    });
    page.drawText(`${((s.amount * s.quantity) / 100).toFixed(2)}`, {
      x: margin + 400,
      y: yPosition,
      size: 12,
      font,
