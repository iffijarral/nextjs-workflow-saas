...
export const EmailFormSchema = z.object({
  receiver: z
    .string()
    .trim()
    .nonempty("Receiver is required")
    .email("Invalid email address"),
  subject: z
    .string({ invalid_type_error: "Please enter a valid subject" })
    .trim()
...
export const CustomerSchema = z.object({
  ...
  type: z.enum(["private", "company"], {
    message: "Customer type must be either 'private' or 'company'.",
  }),
  ...
  status: z.nativeEnum(ProjectStatus, {
    errorMap: (issue) => {
      if (issue.code === z.ZodIssueCode.invalid_enum_value) {
        ...
        return {
          message: `Invalid status. Must be one of: ${Object.keys(ProjectStatus).join(", ")}.`,
        };
      }
...
export const UpdateProjectAssignmentSchema =
  ProjectAssignmentSchema.partial().extend({
    id: z.string(),
  });
