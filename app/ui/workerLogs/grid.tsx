...
export default function WorkLogGrid({
  ...
  const createGridState = (
    entries: FetchedWorkEntry[],
  ): Record<string, EntryCell> => {
  ...
  const projectStartDate = currentProject?.startDate
    ? startOfDay(parseISO(currentProject.startDate.toISOString()))
    : null;
  const projectFinishDate = currentProject?.endDate
    ? endOfDay(parseISO(currentProject.endDate.toISOString()))
    : null;
  ...
  if (
    ...
    const totalDays =
      differenceInCalendarDays(projectFinishDate, projectStartDate) + 1;
  ...
  if (allProjectDays.length > 0) {
    ...
    while (currentWeek.length < 7) {
      currentWeek.push({
        key: `pad-end-${weeks.length}-${currentWeek.length}`,
        label: "",
        isWithinRange: false,
      });
    }
  ...
  const handleProjectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    startTransition(() => {
      // Show pending state while navigation happens
    ...
    <form action={formAction}>
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
          ...
          >
            {projects.map((p) => (
              <option key={p.id} value={p.id}>
                ...
                {p.startDate &&
                  p.endDate &&
                  ` (${format(parseISO(p.startDate.toISOString()), "MMM d, yyyy")} - ${format(parseISO(p.endDate.toISOString()), "MMM d, yyyy")})`}
                {!p.startDate && !p.endDate && ` (Dates not set)`}
                {p.startDate &&
                  !p.endDate &&
                  ` (Starts ${format(parseISO(p.startDate.toISOString()), "MMM d, yyyy")})`}
                {!p.startDate &&
                  p.endDate &&
                  ` (Ends ${format(parseISO(p.endDate.toISOString()), "MMM d, yyyy")})`}
              </option>
            ))}
          </select>
        ...
        {currentProject &&
          ...
          !isPending && (
            <div className="text-center py-4 text-red-500">
              Selected project needs both start and finish dates set.
            </div>
          )}
        {currentProject &&
          projectStartDate &&
          ...
          !isPending && (
            <div className="text-center py-4 text-red-500">
              Project finish date must be after or on the start date.
            </div>
          )}
                    ...
                    {week.map(({ key, label, isWithinRange }) => {
                      ...
                      return (
                        <td
                          ...
                          className={`py-4 ${
                            isToday
                              ? "bg-yellow-100 border-2 border-blue-500 rounded-md"
                              : ""
