...
export default function SalaryTab({
  ...
  const [editingPayment, setEditingPayment] = useState<SalaryPayment | null>(
    null,
  ); // This state can be used to pre-fill the modal with existing payment data if needed
  ...
  return (
    <div>
      ...
      <h2 className="text-xl font-semibold mb-4">
        Salary Summary for {selectedYear}
      </h2>
                ...
                <td className="px-4 py-2 text-sm font-semibold">
                  {summary.outstanding < 0 ? (
                    <span className="text-green-600">
                      Overpaid: {Math.abs(summary.outstanding / 100).toFixed(2)}{" "}
                      DKK
                    </span>
                  ) : summary.outstanding > 0 ? (
